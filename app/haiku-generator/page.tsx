  ...
  async function generateHaikuLine(syllableTarget: number): Promise<string> {
    ...
    while (currentSyllableCounter < syllableTarget) {
      ...
      if (currentSyllableCounter > syllableTarget) {
        while (
          currentSyllableCounter > syllableTarget &&
          wordArray.length > 0
        ) {
  ...
  return (
    <Center>
      <Stack w={"70%"} align="stretch" justify="center" gap="md">
        ...
        >
          This application generates a haiku using a process that involves
          selecting words from a word list and assembling them into a
          traditional Japanese poem structure. Here’s a detailed breakdown of
          the process:
          <ul>
            <li>
              <strong>Word List:</strong> The haiku is composed using a word
              list provided by the{" "}
              <a href="https://www.eff.org/">
                Electronic Frontier Foundation (EFF)
              </a>
              . This list is curated to ensure a wide variety of words that are
              suitable for creating meaningful and diverse phrases. The word
              list is loaded from a file located at{" "}
              <code>/data/eff_large_wordlist.txt</code>.
            </li>
            <li>
              <strong>Random Number Generation:</strong> The application uses a
              Cryptographically Secure Pseudorandom Number Generator (CSPRNG) to
              ensure that the words selected are truly random and unpredictable.
              This is achieved through the <code>getRandomNumber(max)</code>{" "}
              function, which generates random indices to select words from the
              list.
            </li>
            <li>
              <strong>Syllable Counting:</strong> Each word is analyzed to count
              its syllables using the <code>syllableCount(word)</code> function.
              This function processes the word to estimate the number of
              syllables based on vowel patterns and common syllable structures.
            </li>
            <li>
              <strong>Haiku Structure:</strong> A haiku traditionally consists
              of three lines with a syllable pattern of 5-7-5. The application
              generates each line by accumulating words until the syllable count
              for that line meets the target (5 or 7 syllables). If the syllable
              count exceeds the target, excess words are removed, and the
              process is adjusted to fit the required structure.
            </li>
            <li>
              <strong>Passphrase Construction:</strong> Each line is constructed
              by selecting words from the word list and formatting them
              according to user preferences. Words can be capitalized, and a
              user-defined divider (e.g., a dash <code>-</code>) is used to
          ...
          <p>
            ...
            (EFF). The EFF is a reputable organization dedicated to defending
            civil liberties in the digital world. Their word list is designed to
            provide a diverse and reliable set of words for generating
            passphrases and poems, ensuring both security and creativity.
          </p>
          <p>
            This method ensures that each haiku is both creative and
            structurally accurate, leveraging a strong random number generator
            and a trusted word list to produce unique and aesthetically pleasing
            poems.
          </p>
        </Alert>
      </Stack>
