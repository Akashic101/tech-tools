...
import {
  crackMethodOptions,
  crackMethodExplanations,
  zxcvbnExplanation,
} from "./crackMethodData";
...
export default function PasswordStrength() {
  ...
  const [crackMethod, setCrackMethod] = useState(
    "offline_fast_hashing_1e10_per_second"
  );

  const crackMethodData = Object.entries(crackMethodOptions).map(
    ([value, label]) => ({
      value,
      label,
    })
  ...
  function estimatePasswordStrength(): void {
    ...
    const explanation =
      crackMethodExplanations[crackMethod] || "No explanation available.";
    ...
    setEstimate(
      `${strengthMessage}<br><br>${explanation}<br><br>${zxcvbnExplanation}`
    );
    ...
    if (timeToCrack.includes("instant")) {
      ...
      timeToCrack.includes("centuries") ||
      timeToCrack.includes("millennium")
    ) {
      setAlertColor("green");
    }
